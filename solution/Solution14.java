...
class Solution {
    ...
    int[] segments = new int[SEG_COUNT]; //①数组初始化语法错误

    public List<String> restoreIpAddresses(String s) {
        if (s.length() < SEG_COUNT || s.length() > SEG_COUNT * 3) {
            return ans;  // 如果字符串长度小于4或大于12，不可能形成有效的IP地址
        } //②添加对字符串长度不足以形成有效 IP 地址的提前返回
        // segments = new int[SEG_COUNT]; 重复操作
        dfs(s, 0, 0);
        return ans;
    }
    ...
    public void dfs(String s, int segId, int segStart) {
        ...
        if (segId == SEG_COUNT) { //③条件判断===会判断引用是否相同
